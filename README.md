# IntelliQuery - Agentic AI Chatbot
...
**IntelliQuery** is an intelligent, end-to-end Retrieval-Augmented Generation (RAG) system powered by agentic AI architecture. It combines dynamic query routing, intelligent document retrieval, and advanced LLM capabilities to provide accurate, context-aware answers to user queries.

The system implements the **Adaptive RAG** pattern — instead of always retrieving from a single fixed source, it classifies each incoming query and adaptively routes it to the most suitable pipeline: indexed document retrieval, general LLM knowledge, or real-time web search. Retrieved context is also graded for relevance, and low-relevance results trigger an automatic query-rewrite + re-retrieval loop before final answer generation. Built with a modular architecture using LangGraph for workflow orchestration and multiple storage backends for scalability.

---

## 🎯 Key Features

### 🧠 Intelligent Query Routing (Adaptive RAG Pattern)
- **Smart Classification**: Automatically routes queries to the most appropriate processing pipeline
- **Three Query Types**:
...
│   │   └── gemini.py                 # Google Gemini initialization
...
| **LLM Provider** | Google Gemini | ~2.0.0 |
...
A: Currently configured for Google Gemini. You can modify `src/llms/gemini.py` to use other providers.
